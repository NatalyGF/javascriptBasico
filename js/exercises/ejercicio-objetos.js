...
const equipoDesarrollo = {
    ...
    tipoEquipo(){
        if (this.integrantes < 4){
            return "El equipo es pequeño";
        }else if (this.integrantes >= 4 && this.integrantes <= 8){
            return "El equipo es mediano";
        }return "El equipo es grande"
    }
}

console.log(equipoDesarrollo.javascript());
console.log(equipoDesarrollo.tipoEquipo());
...
■ Investigá: ¿Cuál es la diferencia entre .replace() y .replaceAll()? 
Probá con un nombre de evento que tenga más de un espacio y usá el método correcto.

*/

const evento = {
    nombre: "Inauguración de nueva oficina",
    fecha: "18/03/2026",
    lugar: "Vueling",
    cuposDisponibles: true,
    nombreEvento(){
        return this.nombre.toLowerCase().replaceAll(" ", "-");
    },
    inscripcion (){
        return this.cuposDisponibles ? "Puede inscribirse" : "Cupos agotados"
    }
...
console.log(evento.inscripcion());
...
habilidades tiene listadas. Agregale otro método que use .includes() con if/else 
para mostrar 'Perfil
...
■ Investigá: ¿Qué hace .includes() cuando el string buscado tiene espacios? 
Probá buscar ' JavaScript' con espacio al principio y usá .trim() para evitar ese problema.

*/

const perfilCV = {
    nombre: "Nata Gutierrez",
    profesion: "Quality Assurance",
    habilidades: "responsable, trabaja bajo presión, ingles medio, organizada, proactiva",
    numHabilidades (){
        return `El número de habilidades del candidato es: ${this.habilidades.split(",").length}`
    },
    compatibilidad(habilidad){
        const habilidadModificada = habilidad.trim().toLowerCase()
        if(this.habilidades.toLowerCase().includes(habilidadModificada)){
            return "Perfil compatible"
        }return "Perfil no compatible"
    }

}

console.log(perfilCV.numHabilidades());
console.log(perfilCV.compatibilidad(" Responsable"));
...
const sistemaFacturacion = {
    cliente: "Nata",
    items: "lapiz:500; lapicero:2500; cuaderno:12000; libro: 55000; borrador:1000",
    descuento: 0.5,
    precioTotal (){
        const arrayItems = this.items.split(";");
        console.log(arrayItems)

    }
}
sistemaFacturacion.precioTotal()